import logo from "../../assets/logo.png";
const Footer = () => {
  return (
    <>
      <hr className="mt-20 mb-5 max-w-screen-xl mx-auto border-gray-600 " />
      <div className="flex w-full justify-around items-center p-8 text-gray-500">
        <div className="mr-52 w-[50%]">
          <a to="/">
            <img src={logo} alt="" />
          </a>
        </div>
        <div className="flex mr-52 gap-52 ">
          <div className="mr-2 flex flex-col gap-4">
            <a href="#">
              <i className="mr-2 ri-linkedin-box-fill"></i>Linkdin
            </a>
            <a href="#">
              <i className="mr-2 ri-twitter-x-fill"></i>Twitter
            </a>
            <a href="#">
              <i className="mr-2 ri-instagram-line"></i>Instagram
            </a>
          </div>
          <div className="flex flex-col">
            <a href="/">Home</a>
            <a href="/distributors">Distributors</a>
            <a href="/soil">Soils</a>
            <a href="/about">About</a>
          </div>
          <div className="flex flex-col">
            <a href="#">Careers</a>
            <a href="#">Press Releases</a>
            <a href="#">Press Kit</a>
          </div>
        </div>
      </div>
      <hr className="max-w-screen-xl mx-auto my-8 border-gray-600 " />
      <div>
        <p className="text-center text-zinc-500 font-semibold">
          © Copyright 2024. All Rights Reserved by Naveen vinod kumar.
        </p>
      </div>
    </>
  );
};

export default Footer;
